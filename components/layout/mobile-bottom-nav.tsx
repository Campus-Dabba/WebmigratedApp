...
export function MobileBottomNav() {
      ...
      <div className="flex flex-col h-full">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-foreground">
                Chef {session?.user?.user_metadata?.full_name || 'Dashboard'}
              </h3>
              <p className="text-sm text-muted-foreground truncate">{session?.user?.email}</p>
              <Badge className="mt-1 bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300">
                Cook
              </Badge>
            </div>
          </div>
        ...
        <div className="pt-4 border-t border-border">
          <p className="text-center text-xs text-muted-foreground">
            CampusDabba Cook Portal v1.0 • Made with ❤️ for cooks
          </p>
        </div>
      </div>
    )
  ...
  // Student/Guest menu content
  const renderStudentMenuContent = () => {
    return (
      <div className="flex flex-col h-full">
        {/* Header */}
        <SheetHeader className="pb-6">
