...
export function CooksList({ selectedState }: CooksListProps) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-2 md:px-0">
      {cooks.map((cook) => {
        // Check if cook has items for today
        const hasItemsToday = hasTodayItems(cook);
        
        return (
          <Card key={cook.cook_id} className="flex flex-col w-full max-w-full">
            <CardHeader className="relative min-h-[180px] md:min-h-[200px] flex flex-col justify-end p-4 md:p-6 text-white">
              <div
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1 rounded-full bg-white/20 backdrop-blur px-2 py-1 text-xs md:text-sm">
            ...
            <CardContent className="pt-2 px-4 md:px-6 pb-1">
              <div className="flex gap-2 flex-wrap">
                <Badge variant="secondary" className="text-xs">{cook.totalorders}+ orders</Badge>
                {cook.certification && (
                  <Badge variant="outline" className="text-xs">{Object.keys(cook.certification)[0]}</Badge>
                )}
              </div>
